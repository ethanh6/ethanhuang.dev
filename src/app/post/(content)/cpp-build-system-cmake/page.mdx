...
<pre>
  💡 Source file (`main.cpp`) → [ Preprocessor ] → preprocessed file (`main.ii`)
  → [ Compiler ] → assembly file (`main.s`) → [ Assembler ] → object file
  (`main.o`) → [ Linker ] (with some libraries if needed) → executable (`a.out`
  or `main`)
</pre>
